...
def show(frame,feature_name,target_name,intercept,slope,r_squared,mse,description):
    ...
    equation_label = tk.Label(info_labels, text=f"Predicted line equation: {target_name} = {intercept:.2f} + {slope:.2f}*{feature_name}",
                                fg="#FAF8F9", bg="#6677B8", font=("DejaVu Sans Mono", 11))
    equation_label.pack(side='top', pady=10, anchor='center')

    rsquared_label = tk.Label(info_labels, text=f"Coefficient of determination (R²): {r_squared:.4f}",
                                fg="#FAF8F9", bg="#6677B8", font=("DejaVu Sans Mono", 11))
    rsquared_label.pack(side='top', pady=10, anchor='center')

    mse_label = tk.Label(info_labels, text=f"Mean Square Error (MSE): {mse:.4f}", fg="#FAF8F9", bg="#6677B8",
