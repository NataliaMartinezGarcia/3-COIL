...
class ScrollApp:
    ...
    def __init__(self, window):
        ...
        self._x = (self._window.winfo_screenwidth() -
                   self._width) // 2  # Centered on width
        self._y = (self._window.winfo_screenheight() -
                   self._height) // 2  # Centered on height
        ...
        my_scroll_bar = ttk.Scrollbar(
            self._main_frame, orient="vertical", command=self._my_canvas.yview)
    ...
    def search_file(self, event=None):
        ...
        if self._file:
            try:
                ...
                # Success message
                messagebox.showinfo(
                    "Success", "The file has been read correctly.")
    ...
    def header(self):
        ...
        header_frame = tk.Frame(
            self._main_frame, bg='#d0d7f2', height=40, width=682)
        header_frame.pack(fill=tk.X, side='top')
        header_frame.pack_propagate(False)

        label = tk.Label(header_frame, text='PATH', fg='#6677B8',
                         bg="#d0d7f2", font=("DejaVu Sans Mono", 13, 'bold'))
        label.pack(side='left', padx=(10, 5), pady=5)

        # Variable to store the path of the selected file and button to select it
        self._file_path.set(
            "  Open a file by clicking 'Open' or load a model by clicking 'Load'  ")
...
class App:
    ...
    def show_data(self):
        ...
        self._table_frame = tk.Frame(
            self._frame, height=400, width=self._scroll_window.window.winfo_width() - 15)  # Establecer dimensiones
        self._table_frame.pack(side=tk.TOP, fill=tk.X, anchor="center")
        # To use height as table height
        self._table_frame.pack_propagate(False)

        # Table with scrollbars where the data appears
        self._table = ScrollTable(self._table_frame)

        self._table.empty_table()  # Clear the table in case it already had data
        # Copy the data from the DataFrame to the table
        ...
        column_selector_frame = tk.Frame(
            self._frame, height=400, width=self._scroll_window.window.winfo_width() - 15, bg='#d0d7f2')
        ...
        self._menu = MenuManager(
            self, column_selector_frame, numeric, self._data, chart_frame)
    ...
    def show_model(self):
        ...
        info_frame = tk.Frame(self._frame, height=self._scroll_window.window.winfo_height(
        ) - 50, width=self._scroll_window.window.winfo_width() - 15, bg='#d0d7f2')
        info_frame.pack(side=tk.TOP, fill=tk.X, anchor="center")
        info_frame.pack_propagate(False)

        model_interface.show(info_frame, feature_name, target_name,
                             intercept, slope, r_squared, mse, description)
