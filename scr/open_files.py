...
def open_sql(file_path):
    ...
    try:
        ...
        engine = create_engine('sqlite:///' + str(file_path))  # Create an engine object
        df = pd.read_sql_table(table_name, engine)  # Load the table into a DataFrame
        engine.dispose()
...
def open_file(file_path):
    ...
    EXTENSIONS = ('.csv', '.xlsx', '.xls', '.db', '.sqlite')  # Possible extensions
